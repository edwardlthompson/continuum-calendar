...
import org.fossify.calendar.continuum.CalendarNotifyPrefs
import org.fossify.calendar.continuum.ContinuumNotify
...
class EditCalendarDialog(
    ...
    init {
        ...
        binding.apply {
            ...
            val logicalId = ContinuumNotify.logicalIdForCalendarId(calendar!!.id ?: -1L)
            val prefs = ContinuumNotify.resolvePrefs(
                ContinuumSettingsSync(activity).loadLocal(),
                logicalId,
            )
            typeNotifyNewEvent.isChecked = prefs.newEvent
            typeNotifyReminder.isChecked = prefs.reminder
            ...
            .apply {
                activity.setupDialogStuff(
                    ...
                    alertDialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                        val title = binding.typeTitle.value
                        val notifyNew = binding.typeNotifyNewEvent.isChecked
                        val notifyReminder = binding.typeNotifyReminder.isChecked
                        ensureBackgroundThread {
                            calendarConfirmed(title, alertDialog, notifyNew, notifyReminder)
                        }
                    }
                }
            }
    ...
    private fun calendarConfirmed(
        ...
        if (calendar!!.id != -1L) {
            val logicalId = ContinuumNotify.logicalIdForCalendarId(calendar!!.id!!)
            val sync = ContinuumSettingsSync(activity)
            val base = sync.loadLocal()
            val map = base.calendarNotifyPrefs.toMutableMap()
            map[logicalId] = CalendarNotifyPrefs(
                newEvent = notifyNew,
                reminder = notifyReminder,
            )
            try {
                sync.pushPatch(base.copy(calendarNotifyPrefs = map))
            } catch (e: Exception) {
                org.fossify.calendar.continuum.ContinuumDiagnostics.e(
                    "Failed saving calendar notify prefs",
                    e,
                )
            }
