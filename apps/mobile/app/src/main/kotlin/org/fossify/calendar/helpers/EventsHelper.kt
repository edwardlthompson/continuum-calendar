...
class EventsHelper(val context: Context) {
    ...
    fun insertEvent(
        ...
        markContinuumLocalPeerPending(event)
    ...
    private fun markContinuumLocalPeerPending(event: Event) {
        if (event.source != SOURCE_SIMPLE_CALENDAR && event.source != SOURCE_IMPORTED_ICS) return
        // Push signal immediately (Drive App Data) so desktop sees the change without waiting.
        ensureBackgroundThread {
            try {
                org.fossify.calendar.continuum.ContinuumLocalEventsSync(context).notifyLocalChanged()
            } catch (_: Exception) {
            }
        }
    }
    ...
    fun getEventsSync(
        ...
        val hideBirthdays = !context.config.showContactBirthdays
        val hiddenBirthdayCalendarIds =
            if (hideBirthdays) {
                try {
                    org.fossify.calendar.continuum.ContinuumBirthdayFilter.birthdayCalendarIds(context)
                } catch (_: Exception) {
                    setOfNotNull(birthDayEventId.takeIf { it != -1L })
                }
            } else {
                emptySet()
            }
        val providerBirthdayIds =
            if (hideBirthdays) {
                try {
                    org.fossify.calendar.continuum.ContinuumBirthdayFilter.providerBirthdayEventIds(context)
                } catch (_: Exception) {
                    emptySet()
                }
            } else {
        ...
        events = events
            ...
            .filterNot {
                hideBirthdays &&
                    org.fossify.calendar.continuum.ContinuumBirthdayFilter.shouldHideEvent(
                        it,
                        hiddenBirthdayCalendarIds,
                        providerBirthdayIds,
                    )
            }
            .toMutableList() as ArrayList<Event>
