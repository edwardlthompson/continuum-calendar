...
fun Context.scheduleNextEventReminder(event: Event, showToasts: Boolean) {
    if (!org.fossify.calendar.continuum.ContinuumNotify.reminderAllowed(this, event)) {
        if (showToasts) {
...
fun Context.notifyEvent(originalEvent: Event) {
    if (!org.fossify.calendar.continuum.ContinuumNotify.reminderAllowed(this, originalEvent)) {
        return
    }
...
fun Context.getEventListItems(
    ...
    sorted.forEach {
        ...
        if (code != prevCode && addSectionDays) {
            ...
            val day = Formatter.getAgendaSectionTitle(this, code, isToday)
            val listSectionDay = ListSectionDay(day, code, isToday, !isToday && it.startTS < now)
            listItems.add(listSectionDay)
            prevCode = code
        }

        val title = if (config.redactTitlesInScreenshots) "••••••••" else it.title
        val listEvent =
            ListEvent(
                it.id!!,
                it.startTS,
    ...
    if (!addSectionDays) return listItems

    val continuum = org.fossify.calendar.continuum.ContinuumSettingsSync(this).loadLocal()
    val agendaRangeDays = continuum.agendaRangeDays.coerceIn(7, 90)
    val rangeStart = todayCode
    val defaultEndTs = now + agendaRangeDays * 24L * 3600L
    val rangeEnd = if (sorted.isNotEmpty()) {
        val last = Formatter.getDayCodeFromTS(sorted.maxOf { it.startTS })
        val defaultEnd = Formatter.getDayCodeFromTS(defaultEndTs)
        if (last > defaultEnd) last else defaultEnd
    } else {
        Formatter.getDayCodeFromTS(defaultEndTs)
    }

    return org.fossify.calendar.continuum.AgendaEmptyDays.fill(
        context = this,
        items = listItems,
        showEmptyDays = config.showEmptyDaysInAgenda,
        rangeStartCode = rangeStart,
        rangeEndCode = rangeEnd,
        todayCode = todayCode,
        nowTs = now,
        openTitle = getString(org.fossify.calendar.R.string.continuum_open_day),
        redactTitles = config.redactTitlesInScreenshots,
        workingHoursEnd = continuum.workingHours.end,
    )
}
...
fun Context.editEvent(event: ListEvent) {
    // Continuum “Open” day placeholders use id <= 0 — open create-event for that day.
    if (event.id <= 0L) {
        launchNewEventIntent(Formatter.getDayCodeFromTS(event.startTS))
        return
    }
