...
import org.fossify.calendar.continuum.ContinuumSettingsPoller
import org.fossify.calendar.continuum.ContinuumSettingsSync
import org.fossify.calendar.continuum.ContinuumSyncWorker
import org.fossify.calendar.continuum.ContinuumTokenExchangeWorker
...
class MainActivity : SimpleActivity(), RefreshRecyclerViewListener {
    ...
    private var continuumSettingsPoller: ContinuumSettingsPoller? = null
    ...
    override fun onResume() {
        ...
        if (continuumSettingsPoller == null) {
            continuumSettingsPoller = ContinuumSettingsPoller(this) {
                updateViewPager()
                updateWidgets()
            }
        }
        continuumSettingsPoller?.start()
        finishPendingContinuumOAuthIfNeeded()
    }

    /** Oppo often blocks sockets during AuthActivity; complete exchange once MainActivity is up. */
    private fun finishPendingContinuumOAuthIfNeeded() {
        val auth = ContinuumGoogleAuth(this)
        if (auth.isSignedIn() || auth.loadPendingExchange() == null) return
        ContinuumTokenExchangeWorker.enqueue(this)
        ensureBackgroundThread {
            try {
                Thread.sleep(2_000)
                ContinuumHttp.waitForInternetNetwork(this, timeoutMs = 20_000L)
                val pending = auth.loadPendingExchange() ?: return@ensureBackgroundThread
                auth.exchangeCode(
                    pending.clientId,
                    pending.redirectUri,
                    pending.code,
                ...
                ContinuumSettingsSync(this).pushLocalAfterAuth()
                ContinuumLocalEventsSync(this).reconcilePeerRemote()
                ContinuumDiagnostics.i("MainActivity completed deferred OAuth token exchange")
                runOnUiThread {
                    toast(R.string.continuum_peer_sync_connected_toast)
                }
            } catch (e: Exception) {
                ContinuumDiagnostics.e("MainActivity deferred OAuth failed", e)
            }
        }
    }
    ...
    private fun setupOptionsMenu() = binding.apply {
        ...
        mainMenu.requireToolbar().setOnMenuItemClickListener { menuItem ->
            ...
            when (menuItem.itemId) {
                ...
                R.id.continuum_copy_free_slots -> copyContinuumFreeSlots()
                R.id.continuum_propose_times -> copyContinuumProposedTimes()
                R.id.continuum_jump_to_free -> jumpToContinuumFreeBlock()
    ...
    private fun checkIsOpenIntent(): Boolean {
        ...
        if (eventIdToOpen != 0L && eventOccurrenceToOpen != 0L) {
            hideKeyboard()
            // Continuum widget “Open” rows use negative ids — create event for that day.
            if (eventIdToOpen < 0L) {
                launchNewEventIntent(Formatter.getDayCodeFromTS(eventOccurrenceToOpen))
            } else {
    ...
    private fun copyContinuumFreeSlots() {
        ContinuumScheduling.copyFreeSlots(this) {
            runOnUiThread { toast(R.string.continuum_copied_free_slots) }
        }
    }

    private fun copyContinuumProposedTimes() {
        ContinuumScheduling.copyProposedTimes(this) {
            runOnUiThread { toast(R.string.continuum_copied_propose_times) }
        }
    }

    private fun jumpToContinuumFreeBlock() {
        ContinuumScheduling.jumpToNextFree(
            context = this,
            onEmpty = { runOnUiThread { toast(R.string.continuum_no_free_block) } },
            onLaunch = { intent -> runOnUiThread { startActivity(intent) } },
        )
    }
