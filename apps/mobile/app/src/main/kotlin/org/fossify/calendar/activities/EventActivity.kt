...
class EventActivity : SimpleActivity() {
    ...
    private var mAllowConflictSave = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Continuum: skip Fossify commons sideload scare dialog (renamed applicationId).
    ...
    private fun saveEvent() {
        ...
        if (!mIsAllDayEvent && !mAllowConflictSave && mEvent.availability != Attendees.AVAILABILITY_FREE) {
            ensureBackgroundThread {
                val settings = ContinuumSettingsSync(this).loadLocal()
                val from = mEvent.startTS - 24L * 3600L
                val to = mEvent.startTS + 14L * 24L * 3600L
                eventsHelper.getEvents(from, to, applyTypeFilter = true) { events ->
                    val overlaps = ContinuumConflict.findOverlaps(
                        events = events,
                        startTS = mEvent.startTS,
                        endTS = mEvent.endTS,
                        excludeId = mEvent.id,
                    )
                    if (overlaps.isEmpty()) {
                        runOnUiThread { finishSaveEvent(wasRepeatable) }
                        return@getEvents
                    }
                    val busy = ContinuumScheduling.toBusyRanges(
                        events.filter { it.id == null || it.id != mEvent.id },
                    )
                    val suggestion = ContinuumConflict.suggestFreeSlot(
                        busyRanges = busy,
                        startTS = mEvent.startTS,
                        endTS = mEvent.endTS,
                    ...
                    runOnUiThread {
                        ContinuumConflict.showSaveDialog(
                            activity = this,
                            overlaps = overlaps,
                            suggestion = suggestion,
                            onUseSuggestion = { slot ->
                                mEventStartDateTime = Formatter.getDateTimeFromTS(slot.startTs)
                                mEventEndDateTime = Formatter.getDateTimeFromTS(slot.endTs)
                                mEvent.startTS = slot.startTs
                                mEvent.endTS = slot.endTs
                                updateStartTexts()
                                updateEndTexts()
                                mAllowConflictSave = true
                                finishSaveEvent(wasRepeatable)
                            },
                            onSaveAnyway = {
                                mAllowConflictSave = true
                                finishSaveEvent(wasRepeatable)
