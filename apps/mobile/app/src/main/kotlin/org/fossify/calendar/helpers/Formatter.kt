...
object Formatter {
    ...
    /**
     * Continuum agenda day header: includes month (and “Today · …” for the current day).
     * Example: "Today · Tuesday, August 11" or "Wed · August 12".
     */
    fun getAgendaSectionTitle(context: Context, dayCode: String, isToday: Boolean): String {
        val date = getDateFromCode(context, dayCode)
        val weekday = getDateTimeFromCode(dayCode).toString(DAY_OF_WEEK_PATTERN)
        return if (isToday) {
            context.getString(
                org.fossify.calendar.R.string.continuum_today_section_title,
                "$weekday, $date",
            )
        } else {
            "$weekday · $date"
