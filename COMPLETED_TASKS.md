...
- ✅ [AGENT] Recurrence edit scope, richer monthly RRULE, search/jump/year/hotkeys, holiday pack, tray remaining-today count

## Sprint CC3 — Event editor + month today (2026-08-21)

- ✅ [AGENT] Red today surround on desktop FullCalendar month + Android `MonthView`
- ✅ [AGENT] Shared RRULE helpers + desktop EventEditor parity fields (repeat/TZ/reminders/busy/color)
...
- ✅ [AGENT] F-006: Fix shared agenda `toDateKey`/`addDays` to local calendar dates; extend `agenda.test.ts`
- ✅ [AGENT] F-005: Desktop notifications — rebuild timers after load; serialize toggle vs `rescheduleReminders`
- ✅ [AGENT] F-004: Android `ContinuumGoogleAuth.refreshAccessToken` + use from settings/local-events/sync workers
- ✅ [AGENT] F-007 + F-008: Gate `ContinuumNotify` on notifications enabled; single-flight `ensureFreshTokens` on desktop
- ✅ [AGENT] Docs: public OAuth / no client secret in bundles (`docs/GOOGLE_API_SETUP.md`) + Dependabot audit snapshot (`docs/SECURITY_TRIAGE.md`)
- ✅ [AGENT] F-012: Exempt composition-root `App.tsx`/`main.tsx` from static-data line-limit scan
